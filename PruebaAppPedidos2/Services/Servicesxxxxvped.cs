...
namespace PruebaAppPedidos2.Services
{
    public class Servicesxxxxvped
    {
        ...
        public static async Task<ObservableCollection<Modelxxxxvped>> getPedidosFiltrados(string nit, string nombre, DateTime fechaDesde, DateTime fechaHasta)
        {
            var conexionBD = await DataConexion.conectar();
            string fechaDe = fechaDesde.ToString("yyyy-MM-dd");
            string fechaHa = fechaHasta.ToString("yyyy-MM-dd");
            ObservableCollection<Modelxxxxvped> pedidosVendedor = new ObservableCollection<Modelxxxxvped>();
            try
            {
